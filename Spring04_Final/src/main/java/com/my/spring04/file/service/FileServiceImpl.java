package com.my.spring04.file.service;
...
import com.my.spring04.exception.NotDeleteException;
import com.my.spring04.file.dao.FileDao;
...
public class FileServiceImpl implements FileService{
	...
	public void deleteFile(int num, HttpServletRequest request) {
		//삭제할 파일의 정보 얻어오기
		FileDto dto = dao.getData(num);
		//글 잘성자와 로그인된 아이디가 일치하는지 확인해서 일치하면 삭제, 일치하지 않으면 예외 발생
		String id = (String)request.getSession().getAttribute("id");
		if(!dto.getWriter().equals(id)) {
			//예외를 발생시키면 해당예외를 처리하는 곳으로 실행의 흐름이 넘어감
