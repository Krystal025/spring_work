...
import com.my.project01.dto.UsersDto;
import com.my.project01.service.UsersService;
...
public class UsersController {
	...
	public ModelAndView login(ModelAndView mView, UsersDto dto, String url, HttpSession session) {
		...
		//로그인 후에 가야할 목적지 정보(인코딩한 것+인코딩안한 것)를 모두 ModelAndView 객체에 담음
	...
	public ModelAndView info(HttpSession session, ModelAndView mView) {
		//서비스에 필요한 참조값을 전달해서 정보를 가져오는 로직을 처리함  
	...
	public ModelAndView pwdUpdate(UsersDto dto, ModelAndView mView, HttpSession session) {
		//서비스에 필요한 참조값을 전달해서 비밀번호 수정 로직을 처리함
	...
	public Map<String, Object> profileUpload(HttpServletRequest request, MultipartFile image){
		//service를 이용해서 이미지를 upload폴더에 저장하고 리턴되는 Map을 json문자열로 응답
